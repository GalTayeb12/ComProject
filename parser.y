...
func_header
  : DEF IDENTIFIER '(' parameters ')' ':' RETURNS ret_type {
	 ...
	 if (strcmp(params->name, "PARS NONE") != 0) {
      ...
      add_function($2, param_count, params, $8->name); 
  ...
  | DEF IDENTIFIER '(' parameters ')' ':' {
      ...
      add_function($2, param_count, params, NULL); 
      $$ = create_node($2, 2, params, ret_type_node);
  }
;
...
var_single_decl
  ...
  | TYPE type ':' IDENTIFIER ':' expr ',' IDENTIFIER ';' {
      add_var($4, $2->name);  
      add_var($8, $2->name);  
      
      char temp1[100], temp2[100];
      sprintf(temp1, "%s %s", $2->name, $4);
      sprintf(temp2, "%s %s", $2->name, $8);
      
      Node* var1_node = create_node(temp1, 0);
      Node* var2_node = create_node(temp2, 0);
      Node* assign_node = create_node("=", 2, var1_node, $6);
